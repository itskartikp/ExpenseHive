      ...
      /*  const eachExpense = utils.eachExpense(expenses);
       ...
       /* const TotalInThatDay = allDatesInSelectedMonth.map(date => {
        ...
        for (let i = 0; i < moment(allDatesInSelectedMonth[0]).day(); i++) {
            prevDays.push(moment(allDatesInSelectedMonth[0]).subtract(i + 1, 'd').format("MM/DD/YYYY"))
            dayStart.push(i);
        }
        console.log("%c day start ", "background: red; color: white", prevDays)
        ...
        for (
            ...
            nextDays.push(moment(allDatesInSelectedMonth[allDatesInSelectedMonth.length - 1]).add(i + 1, 'd').format("MM/DD/YYYY"))
            dayEnd.push(i);
        }

        console.log("%c day end ", "background: blue; color: white", nextDays)
